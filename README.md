...
> This is an updated version on [ZanySoft/laravel-zip](https://github.com/zanysoft/laravel-zip)
...
use `SamBenne\Zip\Zip` class only use `Zip` is made to handle a zip file.
...
The `\SamBenne\Zip\ZipManager` can handle multiple `SamBenne\Zip\Zip` objects.

- include the Zip and ZipManager class at top:
```php
use SamBenne\Zip\ZipManager;
use Zip;
```
